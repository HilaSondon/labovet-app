...
function ManualAnimalEntry({producer,workIndex,onClose,onSave}:{producer:Producer;workIndex:number;onClose:()=>void;onSave:(producer:Producer)=>void}){
  ...
  const save=()=>{if(!valid.length)return;const works=producer.works.map((w,i)=>i===workIndex?{...w,animals:`${valid.length} animales`,status:"Listo para SIGATM"}:w);onSave({...producer,works})};
