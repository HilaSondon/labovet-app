...
function Protocols({ uid, protocols, sampleIntakes, loading, search, setSearch }: { uid: string; protocols: LaboratoryProtocol[]; sampleIntakes: LaboratoryManagementRecord[]; loading: boolean; search: string; setSearch: (value: string) => void }) {
  ...
  const [selected, setSelected] = useState<LaboratoryManagementRecord | null>(null);
  ...
  const analysisNames = (item: LaboratoryManagementRecord) => {
    try { return (JSON.parse(String(item.analyses || "[]")) as Array<{ name?: string }>).map((value) => value.name).filter(Boolean).join(" · ") || "Sin análisis"; }
    catch { return "Sin análisis"; }
  };
  return <><header className="topbar module-topbar"><div><span className="eyebrow">LABORATORIO</span><h1>Protocolos</h1><p>Ingresos recibidos y protocolos GRECERT procesados, reunidos en un solo lugar.</p></div></header><section className="panel laboratory-list-panel"><div className="laboratory-list-toolbar"><div><h2>{intakes.length + protocols.length} protocolos guardados</h2><p>Seleccioná un ingreso para completar el trabajo administrativo y técnico.</p></div><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar protocolo…" /></div><div className="laboratory-protocol-list-head"><span>Fecha</span><span>Protocolo</span><span>Veterinario</span><span>Productor / RENSPA</span><span>Análisis</span><span>Muestras</span><span>Estado</span></div>{loading ? <div className="laboratory-empty-filter">Cargando protocolos…</div> : intakeVisible.map((item) => <button type="button" className="laboratory-protocol-row intake clickable" key={item.id} onClick={() => setSelected(item)}><time>{String(item.date || "—")}</time><b>{String(item.protocol || "Sin número")}<small>Ingreso de muestras</small></b><span>{String(item.veterinarian || "—")}</span><span>{String(item.client || "—")}<small>{String(item.renspa || item.establishment || "")}</small></span><span>{analysisNames(item)}</span><strong>{String(item.quantity || 0)}</strong><span><em>{String(item.status || "Recibida")}</em><small>Abrir protocolo →</small></span></button>)}{visible.map((item) => <article className="laboratory-protocol-row" key={item.id}><time>{item.date || "—"}</time><b>Informe {item.reportNumber}<small>Acta {item.actNumber}</small></b><span>{item.veterinarianName}<small>{item.veterinarianCuit}</small></span><span>{item.renspa}</span><span>{item.assayName}</span><strong>{item.sampleCount}</strong><span><em>{item.positiveCount} positivos</em><small>{item.suspiciousCount} sospechosos</small></span></article>)}{!loading && !intakeVisible.length && !visible.length && <div className="laboratory-empty-filter">No hay protocolos que coincidan.</div>}</section>{selected && <IntakeProtocolWorkspace uid={uid} record={selected} onClose={() => setSelected(null)} onSaved={(record) => { setIntakes((current) => current.map((item) => item.id === record.id ? record : item)); setSelected(record); }} onDeleted={(id) => { setIntakes((current) => current.filter((item) => item.id !== id)); setSelected(null); }} />}</>;
}
