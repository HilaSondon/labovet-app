...
export default function LaboratoryGreCertPanel({ uid, section = "load" }: { uid: string; section?: LaboratorySection }) {
  ...
  const exportPdf = async () => {
    ...
    autoTable(pdf, { startY: 139, margin: { left: 12, right: 12, bottom: 17 }, head: [["#", "Identificación", "Categoría", "Edad", "Resultado", "Antígeno / Kit", "Marca", "Lote", "Vencimiento", "Estampilla"]], body: rows.map((row) => [row.number, row.identification, CATEGORIES[row.category] || row.category, row.age === "10" ? "N/A" : row.age, RESULTS.find((option) => option.code === row.resultCode)?.label || "Pendiente", row.antigen, row.brand, row.batch, row.expiration, row.stamp]), styles: { fontSize: 7, cellPadding: 2 }, headStyles: { fillColor: [18, 63, 82] }, alternateRowStyles: { fillColor: [247, 250, 250] } });
